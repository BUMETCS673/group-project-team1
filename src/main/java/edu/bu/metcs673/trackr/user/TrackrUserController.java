 ...
 * application. Also contains an API which generates a new JWT token for the user to use for
 * authentication.
 *
 * @author Tim Flucker
 */
...
public class TrackrUserController {
    ...
    /**
     * Using provided request body, creates a new record in the USERS table. Request body is
     * validated, and an encrypted password value is saved. Returns a success message with a JWT
     * token.
     *
     * @param userInput
    ...
    /**
     * Using the provided valid request body, authenicate the user and if credentials are correct,
     * then provide a new JWT token for other API use.
     *
     * @param userLogin
     * @return
     */
    ...
    public ResponseEntity<GenericApiResponse> login(
            @Valid @RequestBody TokenRetrievalDTO userLogin,
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(GenericApiResponse.errorResponse(CommonConstants.INVALID_CREDENTIALS));
        }
    }
